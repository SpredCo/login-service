...
describe('Testing google registration/connection', function () {
  ...
  it('Should reply an error when user is not registred', function (done) {
    loginSrv
  ...
  it('Should register a new user from google', function (done) {
    loginSrv
      .post('/v1/users/google')
      .send({ 'access_token': googleToken, 'pseudo': fixture.googleUser.pseudo })
      .set('Content-Type', 'application/json')
      .auth(fixture.client.key, fixture.client.secret)
      .expect(201)
      .end(function (err, res) {
        if (err) {
          ...
          expect(res.body.pseudo).to.equal(fixture.googleUser.pseudo);
          done();
        }
      });
  });
